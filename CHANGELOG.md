...
- Export waits for pending edits to save before generating the document and reports failures without stray placeholder text.
...
- **A designed voice is played before it becomes anybody's.** Designing gives you the voice to listen to, with **Keep this voice** and **Try again** beside it, and nothing is stored or cast until you keep it. Voice design is not deterministic - the same description asked for twice gives two different voices, and sometimes neither is the one you meant - so the first result was becoming a character's voice until somebody noticed and undid it. Now a miss costs one more press.

- **A reading no longer stops between stretches.** Speech was prepared a windowful at a time and the next windowful was not asked for until the last one had finished playing, so the reading fell silent at every seam while the engine caught up. The next stretch is now rendered while the current one is being spoken, and the stretches grow from small to full size so the early ones cannot fall behind either.

- **A reading starts speaking almost at once, and says so while it gets ready.** Speech was made a dozen lines at a time and none of it played until all twelve were finished, so pressing Play meant ten or twenty seconds of silence with nothing on screen to say anything was happening. The first request is now small, so a voice starts within a second or two and the rest is prepared while it is still speaking - and the transport says **Preparing the reading** until the first sound arrives, rather than sitting there looking like a button that did not work.

- **Play no longer stops the reading it has just started.** Pressing Play begins by stopping, so that a second press replaces the first rather than overlapping it - and stopping clears the rendered audio and cancels whatever is being spoken. Because stopping is deliberately allowed to jump the queue, so that Stop is immediate, the stop could land a moment *after* the new reading had begun: it cancelled that reading and deleted its first clip. The result was a spinning graphics card and silence. Play now waits for the stop it asked for before it starts.

- **A reading is cut into sentences, not into scenes.** Everything between two spoken lines was handed to the speech engine as a single utterance - which in a scene with little dialogue is the whole scene. It meant half a minute of audio from one request, a long wait before any sound at all, far more than a speech model is built to say in one breath, and a follow-along highlight that lit up an entire scene rather than the words being spoken. Prose is now read a sentence at a time, with paragraph breaks always ending one, full-width stops recognised so a Chinese scene is cut like any other, and a sentence that runs on for hundreds of characters broken between words rather than sent whole.
...
- **Directing a line by hand, for the delivery no word covers.** Behind the sixteen emotions there are now eight sliders — happy, angry, sad, afraid, disgusted, melancholic, surprised, calm — that set a line's delivery exactly. They open on what will actually be performed rather than on zero, and what you set is what is used: not rescaled by the scene's intensity, not quietened because the line is narration.
  ...
  **And where nothing is precise enough, point at a line instead.** Any line rendered in this sitting **in that character's own voice** can be picked as a reference, and the engine performs the new one in the manner of that clip — the voice stays the one you designed, only the delivery is borrowed. It is that character's lines and no others: an engine takes its whole delivery from the clip, so pointing one character at another's line would not read it their way, it would read it in their voice. Engines that cannot take a reference fall back to the numbers, so setting one never leaves a line worse off.

- **Settings → Narration.** The speech engines on this machine, what each is doing, and the button that gets one ready — with the download size on it, because it is gigabytes and the decision is yours. Preparing an engine used to be reachable only from a button inside the Narration view, which is the one screen with nothing to show a writer who does not have an engine yet: the way to fix "no speech engine" was to visit the screen that only works once you have one. With none installed, the section says where one comes from.

- **A speech engine can travel in the box.** An edition of Novalist that ships with one copies it into your extensions folder the first time the app runs. An extension you already have at the same version or newer is left exactly as it is, so an update you took from the gallery is never rolled back, and what an engine has downloaded stays where it is rather than being fetched again.

### Fixed

- **Designing a voice again gives you a different voice.** The draw was derived from the words of the brief, so pressing Design a second time on an unchanged description returned the identical voice and said nothing about it — which made "I do not like that one, try again", the thing the whole offer-and-keep dialog exists for, do nothing at all. Every attempt is now its own draw. The brief has a **Seed** field: leave it blank for a fresh voice each time, or put a number in to ask for one particular voice back. What each offered voice was drawn with is shown beside it, with a button to pin it — so a voice you heard once and did not keep is not gone for good.

- **An audiobook is read in paragraphs, not in sentences.** Every sentence was its own call to the speech engine, and a cloning model starts each call afresh from the character's reference clip with no memory of the sentence before it — so pitch, pace and energy reset at every full stop, and a paragraph stitched from four of them sounded like four readings rather than one narrator. Consecutive sentences one voice says are now recorded in one breath, with no invented pause between them. It is measurably slower, not faster; it buys continuity and nothing else.

  Only where every reason to keep them apart is absent: the same speaker, the same direction, never across the quote marks, and never longer than the model actually delivers before it starts cutting off mid-word — measured at around six hundred characters, not assumed. The live reading still goes a sentence at a time, because that is where the highlight follows the voice and correcting a line costs a line.

- **The reading stops stalling every few sentences.** It rendered ahead in windows that **doubled** each time — which quietly demanded an engine twice as fast as speech itself, because while a window of six lines played, one of twelve was being made. Anything slower stalled at every boundary, which is the "a few sentences, a pause, a few more" it sounded like. The window is now a steady size, so a reading only stalls if the engine is slower than real time, and any time it runs ahead is banked instead of spent on a window twice the size.

- **A line already spoken is not spoken again.** Everything made was thrown away the moment you pressed Stop, so listening to a paragraph twice cost twice, and stopping to fix one word made the whole scene again — which is the single commonest thing to do in this view. Each line is now kept under a key made of everything that decides how it sounds: the words, the voice's own audio, the direction, the speed, the engine. Change any of them and that line is made again; change none and it plays instantly. Redesigning a voice counts as a change, so a character never goes on speaking in the voice you just replaced. The kept audio goes when you close the project, and never grows past a couple of gigabytes — what survives is the part you are working on.

- **You can see the reading being built.** Lines already made carry a mark down their edge, and the one line an engine is working on right now is hatched and moving — so a pause reads as a model thinking rather than as a feature that has stopped. It is the line, not the batch: a window is asked for in one go, and marking the whole of it hatched a dozen sentences as "being made" when eleven had not been started. **Read again** in the transport throws the lot away and makes it afresh — delivery is not reproducible, so that is the only way to get a second answer out of an engine.

- **The reading goes to the engine that made the voice.** With two speech engines installed, whichever had finished loading spoke the whole book — so a writer with a real engine and the example tone generator heard their manuscript as a sine wave, and nothing said why, because the reading was doing exactly what it had been told. Which engine speaks is now decided by the voice, per line: a voice can only be spoken by the engine that made it, and a book cast across two of them is read by both, in order. The same for hearing a single line while you write it, and for the audiobook — where the wrong voice is baked into a file and published. Lines cast in a voice whose engine is not installed are reported rather than spoken by a stand-in.

- **You choose which engine designs a voice.** It used to be whichever one had loaded first, which is the example tone generator every time, because it loads instantly and a real speech model takes half a minute. The brief now names the engine when more than one can design, and every designed voice shows the engine it came from — so a voice made by the wrong one is visible instead of only audible.

- **An installed speech engine is ready when you open Novalist.** Its model lives in a process that ends with the app, so "prepared" never survived a restart — and the button that fixed that sits on a rail you have no reason to visit twice. The result was an application that had a speech engine, reported it as not ready every morning, and read the book in the operating system's voice until you found the button again. An engine with everything it needs now starts itself, in the background, and a reading you begin while it is loading waits for it. An engine with a download still outstanding is left alone: that is a decision about your connection, not a startup. Exporting an audiobook loads one too, rather than refusing over a model that simply was not in memory yet.

- **A speech engine no longer asks you to install Python.** It needs an interpreter in a particular range, and a machine with a newer one — or none at all — was told to go and install it, which is homework rather than an installer. It now fetches its own when the machine has nothing suitable, into its own folder, touching nothing you already have. It also no longer settles for an interpreter it knows will fail: that only moved the failure to the end of a two-gigabyte download.

- **Reading aloud works on macOS and Linux.** Only Windows had a speech engine behind it, and elsewhere every request came back refused instantly — so the reading swept through the whole book in about a second, silent, with the highlight racing prose nobody could hear. It looked like a broken feature because it was one. macOS now reads through the voices `say` knows and Linux through espeak-ng where it is installed, both listed in the voice picker under their own names, and the Speed control reaches both. A passage that genuinely cannot be spoken now stops the reading instead of skipping on down the book.

- **A character can sound different in different parts of the book.** One voice per character was one voice for four hundred pages: somebody who ages, is injured, is disguised, or is remembered as a child in an earlier chapter had no way to say so — and designing a second voice for them silently destroyed the first. Each name on the cast rail now offers the stretches your book actually has — every act, every chapter, every scene — and a voice to read them in, resolved most-specific-first the same way the Codex resolves what a character is like at a point in the story. **Design one for here** makes a voice for that stretch alone and keeps the one they use everywhere else. Un-casting a stretch sends those lines back to their standing voice rather than silencing them, and forgetting a voice clears every stretch it was cast over. The narrator gets them too, for a book that hands the telling to somebody else partway.

- **Changing a character's standing register re-renders the audiobook.** It genuinely changed how they were performed, but every chapter already made was reported as unchanged and kept its old delivery — so the only way to hear the change was to delete the render. The join fade counts as a change now too.

- **Updating the speech extension no longer leaves it broken.** The environment it builds recorded only that it had been built once, so a version that needed different packages ran against the previous version's environment and failed on something missing - with no way to reach the install that would have fixed it, because as far as the extension was concerned there was nothing left to do. It now notices when what it needs has changed and rebuilds. Downloaded models are kept.

- **Chinese, Japanese, Korean and Dutch can be chosen as the language you write in.** The picker offered eleven languages while the app knew about more, and the two lists had drifted apart with nothing to notice - so a Chinese novel could not be declared as Chinese, and everything that reads the writing language was told it was English: which analysis pack ran, what an export declared, and which language the book was read aloud in. The list now comes from the one place that decides it.

- **A book written in corner brackets has dialogue in it.** 「 and 」 were not among the quotation marks Novalist recognised, so a Japanese manuscript had no dialogue at all as far as the app was concerned: nobody to cast, nothing to attribute, no lines to direct, and a reading in which the narrator said everybody's words. Nothing reported an error, because every pass thought the book was simply all narration.

- **Hearing a designed voice across its emotional range is reachable.** It was built, it worked, and no button anywhere called it - while the release notes announced it regardless. The design panel now offers it, on a line the character actually speaks.

- **Designing a voice no longer freezes the rest of the app.** The first design fetches a model, and the whole application waited behind it: no scene saved and no view opened for as long as the download took. Preparing an engine was already exempt from that wait; designing a voice, hearing its range, and hearing a single line were not.

- **A German book is read by a German speaker.** Every designed voice carried an English accent, whatever language the book was written in, and the narrator - who speaks most of a novel - had it worst. The accent lives in the short clip a voice is designed as, and that clip was always made by a model trained on English and Chinese only, speaking an English sentence whenever the character had no lines of their own to borrow. A voice is now designed speaking the language of the book it belongs to, by a model that speaks thirty languages, so nothing has to be translated at the moment of reading.

- **A long speech is read as sentences, not as one breath.** Prose between quotes was already cut into sentences; the quoted lines themselves were not. A speech that ran past what the model would say in one go came back cut off mid-word, with nothing to say it had happened - you found out by listening. Speeches are now cut the same way everything else is, and each sentence is spoken, highlighted and cached on its own. A speaker or a direction still belongs to the whole line, because that is what you wrote.

- **A full stop is only the end of a sentence when it is one.** "The bell rang at 10 a.m. sharp." was read as three separate pieces, and so was any sentence containing a title, an initial, a decimal or a time - "Dr. Reyes crossed the room to where Mr. Vance stood." came out in three. Titles, initials, decimals, times and, in German, ordinals like *3. Mai* are now read straight through.

- **Play sounds like the audiobook.** The exported file left a breath between lines and a longer one between scenes; pressing Play in the Narration view left nothing at all, so the preview ran every line into the next and was the worse of the two readings. Both now use the same pauses.

- **The reading no longer skips the click at every join.** The speech extension told Novalist it carried the voice across a line break when it did not, so the short fade that removes the click at each join was skipped - on every join of every exported chapter.

- **The Speed control works on a designed voice.** It was sent to the engine and thrown away, so the slider did nothing once you had designed a voice - while still counting as a change big enough to re-render every chapter of an audiobook you had already made.
...
- **The speech extension runs one model instead of two.** Designing a voice and reading in it were two different models, and the voice that came back was not quite the voice that was designed - the second model re-interpreted the first one's work on the way through. One model now does both, so what you approve is what you hear. It also reads at studio quality, speaks thirty languages, and needs a graphics card with about 8 GB to be comfortable; it still runs without one, and still says so first.
...
- **The command palette finally knows everything.** It used to list only the two dozen commands that happened to carry a keyboard shortcut, which is why Comment, Footnote, Link, Highlight, every alignment and every paragraph style could not be reached by name at all. It now lists every command in the app, and only the ones that can actually do something right now - a command that needs a selection is not offered when there is none. A shortcut is a property of a command rather than the reason it exists, so **Settings**, **Keyboard shortcuts** now lists everything too: you can put a gesture on Comment, on Read aloud, on Clean up the manuscript, on anything.

- **Five workspaces instead of nineteen destinations.** The slim rail of unlabelled icons is replaced by **Write**, **Plan**, **World**, **Publish** and **Series**, each with its name under its icon, plus the way back to the **Dashboard**. Picking one changes the workspace: which views are one click away, and what the window looks like around them. The views themselves live in a labelled list beside the rail - grouped, scrolling, and never hidden. "Where is the Plot Grid" is now "Plan" rather than one of nineteen icons you told apart by hovering. Every view still has its own name, its own hotkey and its own place in the command palette; splitting a pane still puts any view in any pane, so the Codex beside the manuscript works exactly as it did.

  Nothing in a mode is ever put behind a "...". Past ten views the list grows a filter box, but that is an accelerator: every view is in the list whether or not you use it. Views an extension contributes sit in their own group at the end, so installing one never reorders or displaces a view you already knew where to find - which is what the old rail did, and it hid the contributed icons first.
...
- **Interface size.** Settings, Appearance, **Interface size** — how large Novalist draws its own interface, from 75% to 150%, with **Reset to 100%** beside it and `Ctrl+Plus`, `Ctrl+-` and `Ctrl+0` on the same setting. It covers the toolbar, the binder, the inspector, dialogs and Settings itself, and it is deliberately **not** the size of your manuscript: enlarging the interface until you can read its labels no longer drags your prose to a size you never asked for, and setting a large writing font no longer inflates every menu around it. Your operating system's own display scaling still applies on top, so this is the adjustment for when the OS setting suits everything except Novalist. The size you pick is remembered.

- **Help opens where you are.** `F1`, or **Help** then **Help for Current View**, opens the manual at the page for whatever is in front of you — the Timeline page from the Timeline, the Codex page from the Codex, and in Settings the page for the section you have open. A link that points at a particular section now arrives at that section instead of at the top of the page. The full page list and its search are still down the side, so opening in the right place never stops you going somewhere else.

- **Guidance that waits until it means something.** The first-run walkthrough now names one thing to do at each stop — open a scene and write a sentence, set a daily goal you can reach, capture one fact you will want beside the draft — and **Try this** does the part that needs doing where it can. It has stops for the writing view and for Focus Peek, which it previously never mentioned at all, and it ends on Export rather than in the middle of the app. A stop that needs something you do not have says so: with no scene open it tells you to open one, and with no linked names in your prose the Focus Peek stop says that rather than asking you to hover something that is not there.

  Beyond the walkthrough, a short tip is offered the first time a feature actually becomes usable — when your caret first lands on a linked name, for instance. Each one can be tried, put off, or turned off for good, and only which tips you have seen is remembered; nothing about your writing is recorded to decide when to show one. **Settings**, **Accessibility**, **Show contextual guidance while I learn** turns the whole thing off.

- **Display information for support.** Settings, Diagnostics, **Read display information** reports your operating system's scale factor, the current interface size, the window and content size, and the monitor's usable area. It is the fastest way to tell a system scaling problem from a Novalist one, and like the log it contains dimensions only — never a project name, a path, or any of your writing.

- **Imported manuscripts keep their formatting.** Rich text and Scrivener projects now bring across the formatting that carries meaning: bold, italic, underline and strikethrough where they sit inside a sentence, superscript and subscript, headings, block quotations and verse from the named styles the source used, numbered and bulleted lists as real lists, centred and justified paragraphs, and scene-break ornaments as scene breaks. Import used to be prose only, so a book arrived with every emphasis flattened out of it. Page design is still deliberately left behind — typeface, point size, spacing and margins stay in the program you left, because Novalist styles your prose from your own settings. Scrivener research notes keep their formatting too, so a set of headed notes no longer collapses into one block of text.

- **Replacement rules are yours to write.** Settings, Writing assistance, **Replacement rules** — the full list of what gets substituted as you type, editable at last. It was always a stored list; the language preset only ever filled it in, and nothing showed it, so "what gets replaced by what" meant one of eleven presets and nothing else. Add a rule for `->` becoming an arrow, or an abbreviation you keep expanding by hand. A rule can also be a **pattern**: a regular expression that captures part of what you typed and puts it back, so `(\d+)x(\d+)` becoming `$1×$2` turns `12x9` into `12×9` as you finish the word. **Closing** is filled in only for a rule that should alternate the way an opening quote alternates with a closing one. Rules run in the order you list them, so one can act on what another produced, and deleting them all sticks. A pattern that is not valid, or that matches nothing at all, is refused while you are still looking at the row rather than saved and silently skipped; one that turns out too slow to keep up with typing is set aside rather than allowed to stall the editor. **Clean up the manuscript** gained a matching rule, so the rules you wrote can be run over prose already written, patterns and captures included.

  Each rule has a **Try it on** box: type a sentence and watch what that rule does to it as you write the rule, including what a pattern matched and what each capture group holds. It shows typing-time behaviour, so a rule fires where its match ends rather than everywhere at once — which is the part of patterns that is hardest to guess at. A pattern that will not run says so under the row while you are still looking at it. **How matching and replacement work** folds open into the full account, with a short reference for the pattern pieces most rules need.
...
- **An extension can read what your book is about.** The premise and paragraph, the genre, audience, comparable titles and setting, the blurb and the one-page synopsis, the publishing metadata down to the series name and position, and the narrative person and tense you declared. An assistant could read every word of your draft and still had nothing to tell it what kind of book the words were meant to add up to, so it wrote for "a novel" — which is the wrong answer for a cosy mystery and for a literary one alike. Reading only: nothing an extension does can rewrite what your book is meant to be.

- **A character is recognised by their surname.** Novalist matched an entry's full name and a character's bare first name, but never the surname on its own — so “Mira” lit up in your prose and “Frost” did not, which is most of the mentions in any book whose narrator is not on first-name terms with the cast. It works everywhere the name already did: the hover card, the highlighting, and the plural rule. Two characters who share a surname make it ambiguous, so it is left alone rather than pointed at one of them arbitrarily — their given names carry on working.
...
- **Settings is arranged around what you came to change.** Twenty-one sections used to be mounted onto one page that you scrolled, which on a narrow window became a fourth column beside three panels it had nothing to do with. They are now grouped under **General**, **Writing**, **Project** and **System**, and Settings shows **one section at a time**. Search names the individual **controls** that match rather than the sections holding them, and picking a result opens its section and takes you to the control itself; a search that matches nothing says so instead of leaving you an empty page. Every section states its reach on a badge — **All projects**, **Project only**, **Global default**, **Overridden here** — in place of the paragraph that used to re-explain it each time. Settings now opens **without a project**, so your global preferences are reachable from the start screen, with the project-only controls visibly disabled rather than missing. Arriving from a control elsewhere leaves a **Back to...** link to the view you came from.

- **One command, one place.** Novalist grew a command at a time, and each one picked a home on the day it was written, so **Comment** and **Footnote** were reachable from three different places, the five inline marks and the four alignments from two, and the panel toggles from both the toolbar and the View menu. Where a command lives now follows from what it acts on, everywhere in the app: something that acts on **selected text** is in the small toolbar that appears over the selection; on the **thing under the pointer**, in the right-click menu; on the **paragraph or the scene**, on the writing bar above the editor; on the **project**, on the main toolbar; on the **application**, in the menu bar. Everything is additionally in the command palette, and nothing is in two places at once. The build now refuses a change that puts it there.

- **The writing bar holds still.** It used to swap its whole contents depending on whether anything was selected, so the row of buttons you had just learned moved out from under you the moment you dragged across a word. It now holds what acts on the paragraph you are in - style, lists, alignment - plus the scene's snapshots, suggestion mode and **Writing options**, and that set does not change as you select. Bold, italic, underline, strikethrough, highlight, link, **Comment** and **Footnote** live in the toolbar that appears over a selection, and nowhere else. Alignment is gone from that toolbar, because alignment applies to the paragraph rather than to what you selected. **Peek at entity under caret** moved from the bar into the right-click menu and onto `Ctrl+Shift+E`, so it no longer appears and disappears as the caret passes names.

- **The workspace belongs to the mode.** The binder, the inspector and the scene notes are Write's, where there are chapters and scenes to act on. Plan and World keep the book and draft selectors and the status bar; Publish keeps the status bar; Series and the application screens keep none of it. The export view already has its own way of choosing what goes in, so a second copy of the chapter tree beside it was one way too many. Nothing was removed - every command is still in the command palette and on its hotkey, and turning a panel back on leaves your writing layout exactly as you had it.
...
- **Controls say what they are.** **Cmt** and **Fn** in the editor are now **Comment** and **Footnote**. The status bar's row of abbreviations — `12 ch`, `48 sc`, `9 loc` — is gone; the totals it held live behind a single **Project status** button, spelled out as chapters, scenes, characters and locations, together with goal progress and the per-chapter breakdown. The permanent strip of numbers was something to read past while writing rather than something to act on.
...
- **Bold, italic and underline have the gestures every other writing program gives them.** On Windows and Linux none of the three worked. `Ctrl+B` toggled the binder; `Ctrl+I` and `Ctrl+U` were intercepted by the editor, which suppressed what the browser would have done and then handed them to nothing — so they did nothing at all while the buttons beside them said otherwise. All three now apply the mark. The binder moves to `Ctrl+Alt+B` and the inspector to `Ctrl+Alt+I`. On a Mac, `Cmd+B` / `Cmd+I` / `Cmd+U` were always the system gestures and still are. Any of these can be rebound in **Settings**, **Keyboard shortcuts**.
...
- **The backstage drawer** behind the toolbar's burger. Apart from the recent-projects list, which is now in the File menu, it was a second copy of File and Help built when there was no visible menu bar to put them in.
...
- **Looking at something else no longer closes the scene.** Going from the editor to the Timeline, the Codex or anywhere else and coming straight back put you at "Choose a scene in the binder" with your place gone - a two-click detour, and the book was shut. Novalist was treating a pane that had been closed and a pane that was showing something else as the same thing. Your scene is now waiting where you left it, and while you are away the inspector and the status bar go on describing it. The same applies to a split: pointing one half at the Codex and back no longer empties it. Your words were never at risk - they are written out on the way out, as they always were.

- **The comment margin no longer eats the page at a large writing font.** The gutter comments sit in was sized in the editor's own font, so raising the writing font to 36 pixels widened it to most of the window and squeezed the prose into what was left. It is chrome, and is now sized as chrome, independent of how large you set your manuscript. In a narrow pane it steps out of the way entirely.

- **Focus Peek stays put, stays on screen, and comes back when you ask it to.** The card was placed against fixed guesses at its own size, so once its sections had loaded it could hang off the bottom or the side of the pane. It was also positioned against the pointer rather than against the name, so it cleared the single point under the cursor and covered the rest of the word — and the smallest drift of a resting hand then put the pointer on the card, which hid the word from the editor underneath. The peek closed, the name was under the pointer again, and it reopened, several times a second.

  A peek is now measured as it actually renders, anchored to **the whole name**, and placed clear of all of it — below, above, or beside it when the window is too short for either. It holds still while you move around inside the name, stays where it is when you drift off the word and back, and is re-placed when the window or pane changes size. A peek opened from the inspector sits beside its row instead of guessing its own width to get there.

  And clicking a name no longer switches its card off. Put a name in with `@`, or click into one already in the prose to fix a typo, then move the mouse to see who they were — and nothing appeared, until you moved the pointer off the word entirely and came back. That affected `@` mentions and plain names alike.
...
- **Scrivener and rich-text imports no longer arrive corrupted.** A project brought over from Scrivener came in speckled with stray characters where its punctuation should have been: `'93` and `'94` in place of opening and closing quotes, apostrophes and ellipses turned into rubble, em dashes lost, and Scrivener's own internal style markers left sitting in the prose. Anything outside the Latin alphabet fared worse. Smart quotes, apostrophes, ellipses, en and em dashes now arrive as themselves, whatever platform and language the file was written on, and the internal markers never reach your text.

- **An unnamed Scrivener document is no longer an unnamed scene.** Documents you never titled arrive as **Scene 1**, **Scene 2** and so on in binder order, instead of a row of blanks you had to open one by one to tell apart.

- **One unreadable document no longer costs you the whole import.** A file Novalist could not make sense of is now reported on its own and the rest of the import goes ahead.

- **Two settings changes in quick succession no longer lose one of them.** Tabbing straight from one field of a settings form to the next sent two saves at once, and they collided over the settings file: the second was refused outright and the change it carried was gone, with nothing on screen to say so. Saves are now made one at a time, in the order you made them.
...
- **The grammar check no longer holds up everything else.** It posts the scene you are in to a language server and waits for the answer — up to thirty seconds — and while it did, every other screen queued behind it. On a book imported from Scrivener, where a single scene can run to five thousand words, that showed up as scenes taking an age to open: what you were waiting for was waiting on the sentence you had just stopped typing. The check now runs alongside the rest of the app, like version control and backups already did.

- **Novalist works on your project one request at a time now.** Two screens asking for something at the same moment used to read the same half-written answer — which is how the Dashboard came to say its figures could not be worked out, and how Settings came to insist no extensions were installed while five sat on disk. Each was fixed where it showed; this is the reason they kept showing. Work that happens outside your project — version control, backups, exporting — carries on alongside as before, so a slow archive or a git command waiting on a server cannot hold the rest of the app up. Checking whether the app is alive, closing it, and the dialogs an extension puts up are exempt too.

- **The Dashboard refused to load after a trip to Settings.** Opening it, stepping into Settings and coming back left it saying it could not work the figures out — on a book big enough that the first read was still running when the second started. Each reader now works from the running word history as it stood when it started, so a count that arrives a moment out of date is a count rather than a failure. The same fault could break a Dashboard read during an ordinary save.

  Two things around it were wrong as well. While the figures were being worked out the screen claimed to still be connecting to the core process, which by then it is not; and if the read failed there was no message and no way to try again. It now says what it is doing, offers to try again, and keeps the technical detail folded away for a support request.

- **An extension's buttons could vanish although the extension had run.** Extension scripts are loaded when the app starts, when the extensions themselves finish loading, and again when you open the Extensions view, and a reload begins by clearing whatever the previous scripts contributed. Two of those overlapping meant one clearing the status-bar items and commands another had just registered - so the script had run, correctly, and left nothing behind. They run one after another now.
...
- A book can finally get **in** and **out**. Import from Word, OpenDocument, EPUB,
  Markdown, plain text, RTF or a whole Scrivener project; export a finished book with
  its cover, front and back matter, publishing metadata and a print-ready PDF.
- **The window is yours to arrange.** Split the content area into as many panes as you
  want, tear one off into its own window, and save the arrangement by name.
- **Planning became real.** More than one timeline, lanes, spans and date dependencies;
  plot threads as objects rather than ticks; story structure bound to actual scenes; and
  in-world calendars with their own months and eras.
- **The Codex goes deeper.** Every entry type can change through the story, entries keep
  their earlier versions, and there are groups, family trees and a table view.
- **Your work is harder to lose.** Automatic backups with in-app restore, a snapshot
  manager for the whole book, draft comparison, and a recoverable trash.
- **The editor caught up.** Paragraph styles that reach every export, offline spell
  check, read-aloud, readability, suggested edits and pictures in the prose.
- **Extensions can write.** The SDK reads and writes scenes, chapters, projects and Codex
  entries; extensions can run code in the interface and add commands to the palette.
- **Accessibility and comfort.** A High Contrast theme, reading-comfort controls, and an
...
#### The manuscript: chapters, scenes and the binder
...
#### Planning: timelines, plot threads and structure
...
#### The Codex, the world and maps

- **Words you coin are recognised in your prose.** The Languages dictionary was somewhere to go rather than something that helped while drafting — you invented a word in March and looked it up in June by opening a different view. Coined words are now recognised in the editor the way a character's name is: hover one and a card gives its meaning, part of speech, pronunciation and which language it belongs to, and clicking through opens Languages with that word searched. Words shorter than three letters are left alone, and a word that collides with a Codex name loses to the name.
...
#### Research, notes and finding your way around

- **Say what you decided about a note, not just that you are done with it.** Resolving a comment meant only "finished with", so a remark you acted on and a remark you disagreed with came out identical — which is the wrong shape for feedback, where most of what a reader sends you is an opinion to weigh. Each note in the Inbox now carries **Doing**, **Weighing** or **Declining**. Weighing deliberately leaves the note open, because not having decided is a state to sit in. Declining closes it and keeps saying so, which is what you want when a second reader raises the same point six weeks later.

- **The Inspector shows the research this scene is about.** A note saying "check whether the bridge existed in 1755" reached you in the Research view, which means leaving the scene, or on an entity's Wiki article, which means already knowing what to look up — so it stayed filed correctly and unread while the bridge got written. The Inspector's **Research** section now lists what matches the open scene, each with the reason it is there: the character, place or thing it is filed under, or a tag it shares with the scene. Click one to open it. The matching is exact rather than guessed — no AI is involved and no suggestion needs double-checking — and the section is simply absent when nothing matches.
...
#### Goals, analytics and the Style report
...
#### Getting a book in, and getting a book out

- **A Scrivener project now comes across whole.** The import read the binder as one flat list of folders, so the draft folder itself looked like a chapter and every part, chapter and scene beneath it collapsed into a single chapter — while the template's own instruction sheet arrived as prose. It now reads what each part of the binder is from Scrivener's own markers rather than from folder names, so a project in another language, or one where you renamed the draft, imports the same as any other. **Parts become acts**, chapter folders become chapters, and documents become scenes in binder order — including the empty ones, because outlining in empty documents is how most Scrivener projects start. Chapters that share a name stay separate chapters, which the stock novel template needs, since it names every one of them "Chapter". Each scene keeps its **synopsis card, document notes, status, label, custom metadata fields**, and whether it was set to be left out of the compile. **Character and setting sketches become Codex entries**, with the sheet's prose and notes as sections; the blank template sheets are deliberately left behind. Everything else that held content — research notes, front matter, PDFs, pictures, recordings, video — becomes **research copied into your project**, tagged with the folder it sat in, so the Scrivener project can be deleted afterwards. The preview counts all of it before you commit.
...
#### Keeping your work: snapshots, backups and history
...
#### The window, themes and accessibility

- **The content area splits into as many panes as you want.** It used to be one view at a time, with the editor allowed to split in two, so having the manuscript, the Codex and your notes open at once meant picking two and swapping for the third. Any pane can now be split right or down (`Ctrl+Alt+Right`, `Ctrl+Alt+Down`) and split again, and each holds any view. Once a window holds more than one pane, each grows a **header**: the pane's name is a menu of every view — the editor included — so you can point a pane at something else without hunting for the affordance, and the same header carries split, pop-out and close. Drag the divider between two panes to change their proportions. The pane you are working in is outlined, because that is where the activity bar and the command palette land. **Layouts** remembers an arrangement by name — a drafting layout, a revision layout — and puts it back. It names the layout you are in, and always offers **Default**, which collapses the window back to a single pane and cannot be deleted.

- **A pane can be torn off into its own window.** **Open in its own window** — in the toolbar and in every pane header — gives a view a window of its own for a second monitor. It opens on the project the pane came from and, when the editor is what you tore off, on the same scene, so the window arrives showing what you were looking at. It has no activity bar and no binder, but it keeps its pane header, so it can be pointed at another view or split further.

- **Opening a map pin, a research item, or Settings from a link works again.** After the content area learned to split into panes, three kinds of navigation still changed which view Novalist thought was current without changing the one you were looking at — so clicking a research link in the Links panel, a map pin from a bookmark, or a Settings shortcut appeared to do nothing.
...
- **Extension panels can wear the theme you picked.** A panel provided by an extension is a separate document, so it inherited none of Novalist's colours or sizes — every extension shipped its own palette instead, which is why the AI panels were grey and blue inside an app that is ink and gilt, and why choosing Discord or high contrast changed everything except them. Novalist now hands a panel the whole design-token set and re-sends it whenever the theme changes, so a panel can be styled the way the rest of the app is and follow it from then on. Extensions have to ask for it — one line in their page — so a panel that has not been updated keeps the colours its author chose, and the Extensions page in the manual says so. Every panel Novalist's own extensions ship — the AI Assistant's four, plus Insight's report and the Toolkit board — asks for it, so they match the app and follow every theme.

- **Write one section of a Codex entry, or ask for another go at it.** With an AI extension installed, every section head gets a button beside the remove one. It writes that section alone, from everything the entry already holds, using the section's own title as the instruction — so "How they speak" gets an answer about how they speak. Press it again and it is a re-roll: the extension is told what is there and what you did not want, and asked for a different angle rather than the same paragraph rearranged. Until now the only AI over a Codex entry was one whole-entity Wiki summary, regenerated all or nothing, which is the wrong unit for how an entry actually gets filled in — the history is fine, the appearance needs another attempt. Nothing is saved on arrival: the text lands in the editor where you can read it, change it, or click away from it.

- **Commands your extensions register now appear in the command palette.** Extensions could register commands, and nothing ever listed them — an extension author could add "Critique this scene", watch it register, and ship something no writer could reach. The palette now shows them under their own names alongside Novalist's own commands, and running one runs the extension. They are read each time the palette opens, so an extension installed while Novalist is running is usable without a restart.
...
- **Release notes read as notes, not as markup.** The update dialog printed the release text exactly as written, so every asterisk and hash meant to mark a heading or a bold phrase was on screen as a character - and this release is long enough that it mattered. It now renders the notes the way the Extensions store has always rendered them, with real headings and lists to scan.

- **The interface reads at a readable size, and one size per kind of thing.** Sizes had drifted apart until the binder alone ran at four of them - a scene row at one size, the tab above it smaller, its word count smaller again, and its act badge at ten pixels. The scale is rebuilt to nine steps with nothing below thirteen pixels, and each step now answers to a kind of surface rather than to a number: metadata at one size, every control and row at another, prose at the brand size. Buttons, inputs, dropdowns, tabs, chips, menu rows and the status bar were the worst of it - most were wearing the caption size - and they are now a full step larger. The editor's own context menu and floating toolbar match the chrome around them instead of running two steps smaller.

- **The welcome screen stays one screen.** Every project you had ever opened was drawn as a cover, so a dozen of them wrapped into rows that pushed the scratchpad off the bottom. It shows the eight most recent in a single row that scrolls sideways.

- **The split editor is now just panes, and each editor pane holds its own scene.** Two scenes side by side used to be a mode of its own with room for exactly two, and the scene lived in the app rather than in the pane — so splitting the content area while writing gave you the same scene twice, with no way to put a different one in either half. Any pane can be the editor, each with its own scene, its own tab strip and its own auto-save, and there is no limit of two. A pane split off the editor starts empty and waits for a scene: click one in the binder and it opens there. The binder's **Toggle split editor** is now **Open in split**, and a tab's **Move to other split** is **Move to other pane** — it creates one if there is no second pane yet. The inspector, status bar, find-and-replace and scene-notes dock follow the pane you are working in.
...
- **An extension whose tag names it is offered as an update.** A repository can hold several extensions, and then a tag has to say which one it is for - `toolkit-v1.2.0` rather than `v1.2.0`. The store read the version by stripping a leading "v", which left the whole tag and made every such release look like version zero: older than what was already installed, so the update was never offered and nothing said why. The store now reads the version out of the tag whatever it prefixes it with, and can take an extension's compatibility, icon and store page from the release itself rather than from the repository root - which is what lets a repository publish more than one extension without them showing each other's details.

- **The editor's context menu no longer offers a group with nothing in it you can use.** Right-clicking with the caret merely parked still showed the **Codex** group, whose every entry needs selected text — so it opened onto two dead rows, and the group name gave no hint that a selection was what it wanted. A group is now left out entirely when none of its entries can be used; individual rows still grey out, which is what keeps the menu the same shape from one open to the next. Actions contributed by extensions were worse: they appeared fully clickable with nothing selected and then did nothing.

- **The context sidebar no longer vanishes when you click into the pane beside the editor.** It asked what the pane you were in was showing rather than whether the window had the editor open at all, so splitting the manuscript against the Codex and clicking the Codex took the scene's context away. The scene-notes dock did the same. Both now stay for as long as a pane is holding the editor, and follow the editor pane you are working in.

- **Views in a second pane no longer sit on "Connecting to core".** Fourteen views only fetched their data when they were *the* view the window was on, which stopped being true the moment there was more than one pane: a Timeline or a Dashboard beside the manuscript stayed on the connecting message until you clicked into it. Each one now loads as soon as a pane shows it.

- **The family tree shows the family, not one line through it.** It walked straight up from the person you centred on and then straight down, so it could only ever reach the people they descend from and the people who descend from them - no brothers, no aunts, no cousins, no nieces. It now goes up as far as **Generations up** allows and comes back down from everyone it finds, which is what makes it a tree. **Brother**, **sister**, **sibling** and **twin** are read as well, so a family recorded without its parents no longer draws as a single box.

- **The reach and generation dropdowns say what they are.** Three unlabelled boxes reading "2 steps", "3 up" and "3 down" sat in the Relationships toolbar with nothing to say which was which, and the graph's reach control stayed up in tree view where it changed nothing at all. Each now carries its name, and tree view offers its own two generation limits and no third dropdown.

- **A picture lands where you asked for it.** Inserting an image put it at the end of the scene rather than after the paragraph the caret was in. Choosing the file and describing it both take focus away from the prose, so by the time the picture was ready there was no caret left to read - where it goes is now settled at the moment you ask. In page view it was worse: the image went to the end and outside the paper surface, whatever you did.

- **The Layouts drop-down opens.** On Windows and Linux Novalist's own toolbar is the title bar, and every control in it has to be excused from dragging the window. The Layouts drop-down was not, so clicking it moved the window instead of opening the list - which from your side is a control that does nothing. Every control up there is now excused, not only the buttons.

- **The Layouts drop-down says which layout you are in.** It read "Layouts" whatever you had applied, and offered that as an entry you could pick, which then did nothing. It now names the layout the window is currently in, and clears the name by itself once you split a pane, close one or drag a divider - the arrangement is no longer the one that was saved. There is also a **Default** at the top of the list, always offered and not deletable, which puts the window back to a single pane: until now a window you had split up could only be put back a pane at a time. Saving a layout and forgetting one moved into their own **Manage** group, so they cannot be mistaken for layouts by those names.

- **Settings > Accessibility works on a project with its own appearance.** Appearance can be pinned to a project, and its value then overrides the app's. The **High Contrast** button in Accessibility always wrote the app-level setting, so on such a project it did nothing at all, and did it silently: the click was accepted, the setting was written, and the screen never changed. The shortcut now writes wherever that setting lives, like the rest of the section already did.

- **Saving a named layout no longer erases the other kind.** Workspace layouts and pane layouts were both stored under the same name on disk, in two different shapes, so saving one wiped the other and opening the list could land on an entry the other feature had written. They are stored separately now, and applying a workspace layout actually changes the pane you are in rather than only the label for it.
