...
- **The book as data: scene and Codex metadata as CSV or JSON.** Every export so far answered what the book says. Three new ones at the foot of the Export view answer what is in it. **Scenes (CSV)** is one row per scene — stage, point of view, words, target, date, synopsis, goal, conflict, outcome, tags, threads and cast, with scenes you set aside marked rather than dropped. **Codex (CSV)** is one row per field rather than per entry, so a character, a piece of lore and a type you invented yourself all fit the same sheet. **Everything (JSON)** carries both, sections and relationships intact. Cast and threads come out as names, never ids. The spreadsheets are written so Excel reads accented names correctly.
