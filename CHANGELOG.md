...
- **A keyword vocabulary for the book, with colours and a real rename.** Scene tags were free text with nothing behind them, so "flashback", "Flashback" and "flash-back" were three tags and fixing that meant opening every scene that used the wrong one. Settings → Keywords holds the list: each keyword has a colour, can group under another, and says how many scenes carry it. **Collect from scenes** picks up everything already written, folding spelling variants together as it goes. Renaming a keyword rewrites every scene tagged with it in the same moment, and deleting one takes it off those scenes too.
