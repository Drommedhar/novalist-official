...
- **Novalist works on your project one request at a time now.** Two screens asking for something at the same moment used to read the same half-written answer, which is how the Dashboard came to say its figures could not be worked out and Settings came to insist no extensions were installed. Each was fixed where it showed; this is the reason they kept showing. Work that happens outside your project - version control, backups, exporting - carries on alongside as before, so a slow archive or a git command waiting on a server cannot hold the rest of the app up. Checking whether the app is alive, closing it, and the dialogs an extension puts up are exempt too.
...
- **Settings said no extensions were installed while they plainly were.** Novalist decides once, at startup, what is installed - and the flag guarding that decision was set after the work it guarded rather than before, so two screens asking at the same moment both went and looked, and whichever answered second answered from a list the first was still filling. Most often that was an empty one, and the answer stuck until the app was restarted. Asking what is installed now waits for the answer instead of reporting on how far the search has got.

- **The Dashboard refused to load after a trip to Settings.** Opening it, stepping into Settings and coming back left it saying it could not work the figures out - on a book big enough that the first read was still running when the second started. Novalist answers several requests at once and they share one open project, and the running word history was being rebuilt in place while another request was part-way through counting it. Each reader now works from the history as it stood when it started, so a count that arrives a moment out of date is a count rather than a failure. The same fault could break a Dashboard read during an ordinary save.

  Two things around it were wrong as well. While the figures were being worked out the screen claimed to still be connecting to the core process, which by then it is not; and if the read failed there was no message, no way to try again, and it simply stayed there. It now says what it is doing, offers to try again, and keeps the technical detail folded away for a support request.
...
- **Focus Peek cards stay on screen, and stay still.** The card was positioned against fixed guesses at its own size, so once its sections had loaded and it was taller or wider than the guess, it could hang off the bottom or the side of the pane. It is now measured as it actually renders, and re-placed when the window or pane changes size.

  It also flickered, and jumped about. The card was positioned against the pointer rather than against the name, so it cleared the single point under the cursor and covered the rest of the word — and the smallest drift of a resting hand then put the pointer on the card, which hid the word from the editor underneath it. The peek closed, the name was under the pointer again, and it reopened, several times a second. Every time your hand crossed the edge of the name and came back, the card moved to wherever the pointer now was.

  A peek is now anchored to **the whole name**, and placed clear of all of it — below, above, or beside it when the window is too short for either. It holds still while you move around inside the name it belongs to, and stays where it is when you drift off the word and back. Reaching the card with the pointer keeps it open even when it appeared under a cursor that never moved, and a peek opened from the context sidebar sits beside its row instead of guessing its own width to get there.
