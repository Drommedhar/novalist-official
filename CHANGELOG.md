...
- **The writing toolbar shows what applies to where you are.** With nothing selected it is about the paragraph — style, lists, alignment, and **Peek at entity under caret** when the caret is actually on a linked name. With text selected it is about that text: bold, italic, underline, strikethrough, highlight, link, and the two labelled buttons below. The settings that used to sit on the bar permanently — hard-to-read marking, read aloud, page view, paragraph dimming, typewriter scrolling, focus mode — moved into a **Writing options** menu, along with **More writing settings**, which opens the matching part of Settings and offers a link back to the scene. When read aloud or suggestion mode is running, a bar says so and carries one button to leave it. In a narrow pane the primary commands stay put and the rest move into **More** instead of being cut off.

- **The shell shows what belongs to the view.** The binder, the inspector and the scene-creation buttons stay with the writing views, where there are chapters and scenes to act on. Timeline, Codex, Research, Export and the rest keep the book and draft selectors and the status bar and drop the rest; Settings and Extensions keep none of it. Nothing was removed — every command is still in the command palette and on its hotkey, and turning a panel back on in a view that hides it leaves your writing layout exactly as you had it.
