...
The Qwen Speech extension uses MLX on native Apple Silicon Macs for faster local generation. Existing installations need **Prepare** once after upgrading to download the new runtime and model files. Your saved Qwen voices remain usable. Windows, Linux, and Intel Macs continue using PyTorch. On Apple Silicon, normal-speed narration measures how quickly audio is arriving before playback. It starts early only when generation is comfortably faster than playback and enough audio is buffered. Slower or uneven generation finishes the passage first. If a sudden slowdown exhausts the buffer, Novalist buffers the remainder once instead of playing intermittent fragments. The transport shows that the reading is being prepared while it waits. This uses less memory; completed passages are kept for instant replay. At other speed settings, generation still uses less memory, but playback waits for the complete passage to preserve pitch without audible joins.
